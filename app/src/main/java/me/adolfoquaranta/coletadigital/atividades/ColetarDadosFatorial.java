...
public class ColetarDadosFatorial extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final String bloco = "Bloco " + (blocoAtual + 1), fator = fatores.get(fatorAtual).getNome_Fator(), nivelFator = niveisFator.get(nivelFatorAtual).getNome_NivelFator(), repeticao = "Repetição " + (repeticaoAtual + 1), replicacao = "Réplica " + (replicacaoAtual + 1);
        String infoColeta = "";
        if (formulario.getQuantidadeBlocos_Formulario() <= 1 && formulario.getQuantidadeReplicacoes_Formulario() <= 1) {
            infoColeta = fator + " | " + nivelFator + " | " + repeticao;
            quantidadeTotalDados = fatores.size() * niveisFator.size() * formulario.getQuantidadeRepeticoes_Formulario() * variaveis.size();
        } else if (formulario.getQuantidadeBlocos_Formulario() <= 1) {
            infoColeta = fator + " | " + nivelFator + " | " + repeticao + " | " + replicacao;
            quantidadeTotalDados = fatores.size() * niveisFator.size() * formulario.getQuantidadeRepeticoes_Formulario() * formulario.getQuantidadeRepeticoes_Formulario() * variaveis.size();
        } else if (formulario.getQuantidadeReplicacoes_Formulario() <= 1 && formulario.getQuantidadeRepeticoes_Formulario() <= 1) {
            infoColeta = bloco + " | " + fator + " | " + nivelFator;
            quantidadeTotalDados = formulario.getQuantidadeBlocos_Formulario() * fatores.size() * niveisFator.size() * variaveis.size();
        } else if (formulario.getQuantidadeReplicacoes_Formulario() <= 1) {
            infoColeta = bloco + " | " + fator + " | " + nivelFator + " | " + repeticao;
            quantidadeTotalDados = formulario.getQuantidadeBlocos_Formulario() * fatores.size() * niveisFator.size() * formulario.getQuantidadeRepeticoes_Formulario() * variaveis.size();
        } else if (formulario.getQuantidadeRepeticoes_Formulario() <= 1) {
            infoColeta = bloco + " | " + fator + " | " + nivelFator + " | " + replicacao;
            quantidadeTotalDados = formulario.getQuantidadeBlocos_Formulario() * fatores.size() * niveisFator.size() * formulario.getQuantidadeReplicacoes_Formulario() * variaveis.size();
        } else {
            infoColeta = bloco + " | " + fator + " | " + nivelFator + " | " + repeticao + " | " + replicacao;
            quantidadeTotalDados = formulario.getQuantidadeBlocos_Formulario() * fatores.size() * niveisFator.size() * formulario.getQuantidadeRepeticoes_Formulario() * formulario.getQuantidadeRepeticoes_Formulario() * variaveis.size();
            ...
            public void onClick(View view) {
                ...
                if (replicacaoAtual + 1 == formulario.getQuantidadeReplicacoes_Formulario()) {
                    if (repeticaoAtual + 1 == formulario.getQuantidadeRepeticoes_Formulario()) {
                        if (nivelFatorAtual + 1 == fatores.get(fatorAtual).getQuantidadeNiveis_Fator()) {
                            if (fatorAtual + 1 == formulario.getQuantidadeFatores_Formulario()) {
                                if (formulario.getQuantidadeBlocos_Formulario() == -1) {
                                    ...
                                    if (blocoAtual + 1 == formulario.getQuantidadeBlocos_Formulario()) {
                                        ...
                                        recarregar.putExtra("nivelFatorAtual", nivelFatorAtual);
    ...
    private boolean inserirDados() {
        ...
        for (int i = 0; i < dados.size(); i++) {
            ...
            Long checagem = dbAuxilar.checarDadoFat(dados.get(i));
            if (checagem == 0) {
        ...
        if (dbAuxilar.lerTodosDadoDe(id_Coleta).size() != quantidadeTotalDados)
            coleta.setStatus_Coleta(String.valueOf(fatorAtual) + "," + String.valueOf(nivelFatorAtual) + ","
